...
class ReadBufr;
static ReadBufr* instance_ = nullptr;

class ReadBufr : public eckit::Tool {
...
public:
    ...
    ReadBufr(int argc, char **argv) : eckit::Tool(argc, argv) {
...
void ReadBufr::list_all_kvs(codes_handle* h)
{
    // CODES_CHECK(codes_set_long(h, "unpack", 1), nullptr); // tell ecCodes to unpack the data values
...
void ReadBufr::list_namespace_kvs(codes_handle* h)
{
    char value[128] = {0};
    static std::string BufrNamespace = eckit::Resource<std::string>("BufrNamespace", "mars");

    codes_keys_iterator* ks = codes_keys_iterator_new(h, CODES_KEYS_ITERATOR_ALL_KEYS, BufrNamespace.c_str());
    ASSERT(ks);
