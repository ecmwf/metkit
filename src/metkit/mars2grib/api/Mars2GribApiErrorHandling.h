...
/// @file Mars2GribApiErrorHandling.h
/// @brief Common error handling for mars2grib public API calls.
#pragma once
...
inline eckit::PathName errorStackDirectory(const metkit::mars2grib::Options& opts) {
    return eckit::PathName(opts.errorStackPath).fullName();
}

/// @brief Build a unique error-stack file path for an API entry point.
inline eckit::PathName makeErrorStackFilePath(const metkit::mars2grib::Options& opts, const std::string& apiName) {
    ...
    try {
        ...
        throw eckit::UserError("Unable to create Mars2Grib error stack directory `" + dir.asString() + "`: " + e.what(),
                               Here());
    }

    std::ostringstream fileName;

    fileName << "mars2grib-error-stack-" << sanitizeForFileName(apiName) << "-pid" << static_cast<long>(::getpid())
...
///
/// This assumes the mars2grib exception stack is represented using
/// std::nested_exception / std::throw_with_nested. If mars2grib uses a custom
/// nesting mechanism instead, this is the only helper that needs to be adapted.
...
/// @brief Serialize a mars2grib exception stack to disk.
inline void writeStructuredExceptionFrame(const std::exception& exception, std::ostream& out, std::size_t level,
    ...
    if (const auto* me = dynamic_cast<const Mars2GribGenericException*>(&exception)) {
...
/// @brief Serialize a mars2grib exception stack to disk.
inline void writeMars2GribExceptionStackToFile(const Mars2GribGenericException& exception, const std::string& apiName,
                                               const eckit::PathName& errorStackFilePath,
                                               const eckit::CodeLocation& loc = eckit::CodeLocation()) {
    std::ofstream out(errorStackFilePath.asString());

    if (!out) {
        throw eckit::UserError("Mars2Grib conversion failed, but the error stack file could not be opened: `" +
...
/// @brief Print a mars2grib exception stack to standard error.
inline void writeMars2GribExceptionStackToStdErr(const Mars2GribGenericException& exception, const std::string& apiName,
                                                 const eckit::CodeLocation& loc = eckit::CodeLocation()) {
...
inline std::string buildPublicErrorMessage(const InnermostExceptionInfo& inner,
    ...
    os << "Mars2Grib conversion failed. "
...
/// @brief Execute a callable under the mars2grib API error boundary.
///
/// @tparam Result
...
///
/// @param[in] loc
/// Source location attached to the API-boundary frame.
template <typename Result, typename Callable>
Result withMars2GribApiErrorHandling(const std::string& apiName, const metkit::mars2grib::Options& opts,
                                     Callable&& callable, const eckit::CodeLocation& loc = eckit::CodeLocation()) {
    try {
        return std::forward<Callable>(callable)();
    }
    catch (const Mars2GribGenericException& e) {
        const detail::InnermostExceptionInfo inner = detail::innermostExceptionInfo(e, loc);
        const bool printErrorStackToStdErr =
            opts.printErrorStackToStdErr || detail::envVarEnablesStackMode("MARS2GRIB_PRINT_STACK_TO_STDERR");
        const bool saveErrorStack =
            opts.saveErrorStack || detail::envVarEnablesStackMode("MARS2GRIB_WRITE_STACK_TO_FILE");

        if (printErrorStackToStdErr) {
            detail::writeMars2GribExceptionStackToStdErr(e, apiName, loc);
        }
        ...
        try {
            ...
            detail::writeMars2GribExceptionStackToFile(e, apiName, errorStackFilePath, loc);
