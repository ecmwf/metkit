...
/// @brief Matcher and builder for the normal forecast domain.
///
/// This header is the authoritative implementation of the `ForecastDomain` domain case. The matcher identifies the
...
/**
 * @brief Return true only when input matches the normal forecast domain.
 *
...
inline bool match_Forecast_Domain(const ProductTimeSpecInput& input) {
    ...
    try {
        ...
        return isNotSynoptic && isForecast;
    }
    catch (...) {
...
/**
 * @brief End the domain at reference plus step and extend backward by the outer range.
 *
 * @param[in] input Fully normalized ProductTimeSpec input snapshot.
...
inline ProductTimeSpecDomain build_Forecast_Domain(const ProductTimeSpecInput& input,
    ...
    using metkit::mars2grib::utils::exceptions::Mars2GribModelException;
    using metkit::mars2grib::utils::time_arithmetic::addSeconds;
    using metkit::mars2grib::utils::time_arithmetic::subtractDuration;

    try {
        const auto domainEndDateTime = addSeconds(anchor.referenceDateTime, detail::resolvedStepInSeconds(input));
