...
///
/// @file Hindcast.h
/// @brief Matcher and builder for the Hindcast anchor case.
///
...
inline bool match_Hindcast_Anchor(const ProductTimeSpecInput& input) {
    ...
    try {
        ...
        return hasDate && hasHdate && hasNoYearMonth && hasNoPartialYearMonth;
    }
    catch (...) {
...
inline ProductTimeSpecAnchor build_Hindcast_Anchor(const ProductTimeSpecInput& input,
    ...
    try {
        ...
        return checkedAnchor(input, labelDateTime, initialConditionsDateTime, referenceDateTime,
