...
/// @brief Matcher and leaf builder for the AIFS positive-step from-start single-loop shape.
///
/// This header is the authoritative implementation of the
/// `AIFSFromStartSingleLoopPositive` ProductTimeSpec shape. It deliberately owns
...
inline bool match_AIFSFromStartSingleLoopPositive_Shape(
    ...
    try {
        ...
        return isAifs && hasForecastDomain && isNotSynoptic && sourceIncrementIsMissing && usesFromStartTimespan &&
               hasNoStattypeBlocks && stepIsPositive;
    }
    catch (...) {
