 ...
 */
inline std::vector<ProductTimeSpecWindow> build_IFSStandardMultiLoop_Shape(
    ...
    try {
        ...
        std::throw_with_nested(
            Mars2GribModelException("Failed to execute `build_IFSStandardMultiLoop_Shape`", input.to_json(), Here()));
    }
}
