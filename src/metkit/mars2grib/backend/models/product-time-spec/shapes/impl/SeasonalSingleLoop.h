...
inline bool match_SeasonalSingleLoop_Shape(const ProductTimeSpecInput& input, Cntx_t& cntx) {
    ...
    try {
        ...
        {
            bool result = hasSeasonalClass && hasSeasonalStream && hasSeasonalLeadSemantics && isForecast && isNotSynoptic &&
        ...
        std::throw_with_nested(
            Mars2GribModelException("Failed to execute `match_SeasonalSingleLoop_Shape`", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}
...
inline ProductTimeSpecOuterTimeRange build_SeasonalSingleLoop_ShapeOuterTimeRange(
    ...
    try {
        ...
        if (input.stattype.size() != 1) {
            throw Mars2GribModelException("SeasonalSingleLoop requires exactly one stattype block", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!compareTimeDuration(stattype.timeRange, timeRange, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
        ...
        std::throw_with_nested(Mars2GribModelException(
            "Failed to execute `build_SeasonalSingleLoop_ShapeOuterTimeRange`", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}
...
inline ProductTimeSpecShape build_SeasonalSingleLoop_ShapeWindows(
    ...
    try {
        ...
        if (!outerTimeRangeIsAvailable || !outerTimeRange.timeRange.has_value()) {
            throw Mars2GribModelException("SeasonalSingleLoop requires an available outer time range", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!compareTimeDuration(*outerTimeRange.timeRange, timeRange, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
...
inline bool check_SeasonalSingleLoop_Shape(
    ...
    try {
        ...
        if (classification.shapeType != ProductTimeSpecShapeKind::SeasonalSingleLoop) {
            throw Mars2GribModelException("Shape classification mismatch: expected SeasonalSingleLoop", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!hasSeasonalClass || !hasSeasonalStream || !hasSeasonalLeadSemantics || !isForecast || !isNotSynoptic ||
            !hasAcceptedTimespanRepresentation || !hasExactlyOneStattypeBlock) {
            throw Mars2GribModelException("SeasonalSingleLoop input semantics are not satisfied", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!compareTimeDuration(stattype.timeRange, expectedTimeRange, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("SeasonalSingleLoop stattype range must be one calendar month",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }

        if (shape.values.size() != 1) {
            throw Mars2GribModelException("SeasonalSingleLoop shape must contain exactly one window", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!compareTimeDuration(window.timeRange, expectedTimeRange, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("SeasonalSingleLoop window timeRange is inconsistent", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        if (!compareTimeDuration(window.timeIncrement, expectedIncrement.timeIncrement, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("SeasonalSingleLoop window timeIncrement is inconsistent", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                                          Here());
        }
        ...
        std::throw_with_nested(
            Mars2GribModelException("Failed to execute `check_SeasonalSingleLoop_Shape`", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}
