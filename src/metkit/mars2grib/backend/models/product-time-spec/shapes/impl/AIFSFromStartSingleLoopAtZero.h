...
/// @brief Matcher, builders, and checker for the AIFSFromStartSingleLoopAtZero shape.
///
/// This header is the authoritative implementation of the
/// `AIFSFromStartSingleLoopAtZero` shape case. It keeps recognition,
...
/**
 * @brief Return true only when input matches the AIFSFromStartSingleLoopAtZero shape.
 *
 * - the regime is AIFS;
...
inline bool match_AIFSFromStartSingleLoopAtZero_Shape(const ProductTimeSpecInput& input, Cntx_t& cntx) {
...
/**
 * @brief Construct the stage-1 outer range for the AIFSFromStartSingleLoopAtZero shape.
 *
 * In this case:
 ...
 */
template <class Cntx_t>
inline ProductTimeSpecOuterTimeRange build_AIFSFromStartSingleLoopAtZero_ShapeOuterTimeRange(
    ...
    try {
        ...
        std::throw_with_nested(Mars2GribModelException(
            "Failed to execute `build_AIFSFromStartSingleLoopAtZero_ShapeOuterTimeRange`", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}

/**
 * @brief Construct the canonical AIFS from-start zero-step window.
 *
 * In this case:
 ...
 */
template <class Cntx_t>
inline ProductTimeSpecShape build_AIFSFromStartSingleLoopAtZero_ShapeWindows(
    const metkit::mars2grib::backend::models::product_time_spec::ProductTimeSpecInput& input,
    ...
    try {
        ...
        if (!outerTimeRangeIsDeferred || outerTimeRange.timeRange.has_value()) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero requires a deferred outer time range",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }
        ...
        std::throw_with_nested(Mars2GribModelException(
            "Failed to execute `build_AIFSFromStartSingleLoopAtZero_ShapeWindows`", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}

/**
 * @brief Validate one resolved AIFSFromStartSingleLoopAtZero shape against its source input and stage artifacts.
 *
 * This checker verifies:
 * - the resolved shape classification is `AIFSFromStartSingleLoopAtZero`;
 ...
 * - that window matches the locally recomputed zero-step from-start AIFS semantics.
 *
 * @param[in] input Fully normalized ProductTimeSpec input snapshot.
 * @param[in] classification Full resolved ProductTimeSpec classification bundle.
 * @param[in] anchor Previously constructed ProductTimeSpec anchor.
 ...
 * @return `true` when the shape is valid for the AIFSFromStartSingleLoopAtZero case.
 * @throws Mars2GribModelException if the resolved shape is inconsistent with
 *         the input, classification, or case semantics.
 */
template <class Cntx_t>
inline bool check_AIFSFromStartSingleLoopAtZero_Shape(
    ...
    try {
        ...
        if (classification.shapeType != ProductTimeSpecShapeKind::AIFSFromStartSingleLoopAtZero) {
            throw Mars2GribModelException("Shape classification mismatch: expected AIFSFromStartSingleLoopAtZero",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }
        ...
        if (!isAifs || !isForecast || !isNotSeasonal || !isNotSynoptic || !sourceIncrementIsMissing ||
            !usesFromStartTimespan || !hasNoStattypeBlocks || !hasZeroStep || !stepZeroIsAllowed ||
            !operationIsAllowed) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero input semantics are not satisfied",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }

        if (domain.isSynoptic) {
            throw Mars2GribModelException(
                "AIFSFromStartSingleLoopAtZero shape must not be paired with a synoptic domain", input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())),
                Here());
        }
        ...
        if (shape.values.size() != 1) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero shape must contain exactly one window",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }
        ...
        if (window.typeOfStatisticalProcessing != input.innerMostTypeOfStatisticalProcessing) {
            throw Mars2GribModelException(
                "AIFSFromStartSingleLoopAtZero window statistical processing does not match the innermost input "
                "processing",
                input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }

        if (window.typeOfTimeIncrement != missingTypeOfTimeIncrement(metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero window typeOfTimeIncrement is inconsistent",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }

        if (!compareTimeDuration(window.timeRange, expectedTimeRange, metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero window timeRange is inconsistent",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }
        ...
        if (!compareTimeDuration(window.timeIncrement, missingIncrement(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
            throw Mars2GribModelException("AIFSFromStartSingleLoopAtZero window timeIncrement must be missing",
                                          input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here());
        }
        ...
        std::throw_with_nested(Mars2GribModelException("Failed to execute `check_AIFSFromStartSingleLoopAtZero_Shape`",
                                                       input.to_json(metkit::mars2grib::utils::profiling::callSite(cntx, Here())), Here()));
    }
}
