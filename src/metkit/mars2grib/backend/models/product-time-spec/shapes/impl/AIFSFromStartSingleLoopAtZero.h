 ...
 */
inline std::vector<ProductTimeSpecWindow> build_AIFSFromStartSingleLoopAtZero_Shape(
    ...
    try {
        ...
        std::throw_with_nested(Mars2GribModelException("Failed to execute `build_AIFSFromStartSingleLoopAtZero_Shape`",
                                                       input.to_json(), Here()));
    }
}
