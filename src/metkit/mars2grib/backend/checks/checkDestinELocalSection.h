...
void check_DestinELocalSection_or_throw(const OptDict_t& opt, const OutDict_t& out, Cntx_t& cntx) {
            ...
            if (get_or_throw<bool>(opt, "applyChecks", metkit::mars2grib::utils::profiling::callSite(cntx, Here()))) {
                ...
                if (long hasLocalUseSection = get_or_throw<long>(out, "localUsePresent", metkit::mars2grib::utils::profiling::callSite(cntx, Here())); hasLocalUseSection != 0) {
                    ...
                    long actualProductionStatusOfProcessedData =
                        get_or_throw<long>(out, "productionStatusOfProcessedData", metkit::mars2grib::utils::profiling::callSite(cntx, Here()));
