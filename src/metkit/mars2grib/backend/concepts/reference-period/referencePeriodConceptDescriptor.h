...
///
/// @file referencePeriodConceptDescriptor.h
/// @brief Compile-time registry entry for the GRIB `referencePeriod` concept.
///
/// This header defines `ReferencePeriodConcept`, the **compile-time descriptor**
/// that registers the GRIB `referencePeriod` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/reference-period/referencePeriodEncoding.h"
#include "metkit/mars2grib/backend/concepts/reference-period/referencePeriodEnum.h"
#include "metkit/mars2grib/backend/concepts/reference-period/referencePeriodMatcher.h"
...
///
/// @brief Compile-time descriptor for the `referencePeriod` concept.
///
/// `ReferencePeriodConcept` registers the GRIB `referencePeriod` concept into the
/// compile-time registry engine.
///
...
///
struct ReferencePeriodConcept : RegisterEntryDescriptor<ReferencePeriodType, ReferencePeriodList> {
    ...
    ///
    static constexpr std::string_view entryName() { return referencePeriodName; }
    ...
    static constexpr std::string_view variantName() {
        return referencePeriodTypeName<T>();
    }
    ...
    /// callback implementing the `referencePeriod` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (referencePeriodApplicable<Stage, Sec, Variant>()) {
                return &ReferencePeriodOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &referencePeriodMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
