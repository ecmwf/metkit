...
///
/// @file referencePeriodEnum.h
/// @brief Definition of the `referencePeriod` concept variants and compile-time metadata.
///
/// This header defines the **static description** of the GRIB `referencePeriod`
/// concept used by the mars2grib backend. It contains:
///
/// - the canonical concept name (`referencePeriodName`)
/// - the enumeration of supported reference-period variants (`ReferencePeriodType`)
/// - a compile-time typelist of all variants (`ReferencePeriodList`)
...
/// `referencePeriodEncoding.h` implementation.
///
...
///
/// @brief Canonical name of the `referencePeriod` concept.
///
/// This identifier is used:
/// - as the logical concept key in the concept registry
/// - for logging and debugging output
/// - to associate variants and capabilities with the `referencePeriod` concept
///
/// The value must remain stable across releases.
///
inline constexpr std::string_view referencePeriodName{"referencePeriod"};


///
/// @brief Enumeration of all supported `referencePeriod` concept variants.
///
/// Each enumerator represents a specific reference-period classification or
/// processing mode handled by the encoder.
///
...
/// introduced in the future as the reference-period concept evolves.
///
/// @warning
/// Do not reorder existing enumerators, as they are used in compile-time
/// tables and registries.
///
enum class ReferencePeriodType : std::size_t {
    Default = 0
};


///
/// @brief Compile-time list of all `referencePeriod` concept variants.
///
...
///
using ReferencePeriodList = ValueList<ReferencePeriodType::Default>;


///
/// @brief Compile-time mapping from `ReferencePeriodType` to human-readable name.
///
/// This function returns the canonical string identifier associated
/// with a given reference-period variant.
///
...
///
/// @tparam T Reference-period variant
...
///
template <ReferencePeriodType T>
constexpr std::string_view referencePeriodTypeName();

#define DEF(T, NAME)                                          \
    template <>                                               \
    constexpr std::string_view referencePeriodTypeName<T>() { \
        return NAME;                                          \
    }

DEF(ReferencePeriodType::Default, "default");
