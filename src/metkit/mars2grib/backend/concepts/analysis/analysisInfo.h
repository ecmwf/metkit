...
/**
 * @file analysisConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `analysis` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `analysis` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`AnalysisOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `analysis` concept.
 *
 * `AnalysisConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `AnalysisType` variant.
 *
 * @note
...
struct AnalysisConceptInfo {
    ...
    /**
     * @brief Canonical name of the `analysis` concept.
     *
     ...
     * - a pointer to the appropriate `AnalysisOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `AnalysisOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `analysisApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, AnalysisType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of an `analysis` variant.
     *
     ...
     *
     * @tparam Variant Analysis variant identifier
