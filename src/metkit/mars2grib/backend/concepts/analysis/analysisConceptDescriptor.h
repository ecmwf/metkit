...
/// that registers the GRIB `analysis` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/analysis/analysisMatcher.h"
...
///
/// @brief Compile-time descriptor for the `analysis` concept.
///
/// `AnalysisConcept` registers the GRIB `analysis` concept into the
/// compile-time registry engine.
///
...
///
struct AnalysisConcept : RegisterEntryDescriptor<AnalysisType, AnalysisList> {
    ...
    ///
    static constexpr std::string_view entryName() { return analysisName; }
    ...
    /// callback implementing the `analysis` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, AnalysisType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (analysisApplicable<Stage, Sec, Variant>()) {
                return &AnalysisOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    /// The `analysis` concept does not define variant-level callbacks,
    ...
    ///
    template <std::size_t Capability, AnalysisType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    ///
    /// This callback is invoked to determine whether the `analysis`
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &analysisMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
