...
/**
 * @file ensembleConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `ensemble` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `ensemble` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`EnsembleOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `ensemble` concept.
 *
 * `EnsembleConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `EnsembleType` variant.
 *
 * @note
...
struct EnsembleConceptInfo {
     ...
     * - a pointer to the appropriate `EnsembleOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `EnsembleOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `ensembleApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, EnsembleType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of an `ensemble` variant.
     *
     ...
     *
     * @tparam Variant Ensemble variant identifier
