...
/// that registers the GRIB `ensemble` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/ensemble/ensembleMatcher.h"
...
///
/// @brief Compile-time descriptor for the `ensemble` concept.
///
/// `EnsembleConcept` registers the GRIB `ensemble` concept into the
/// compile-time registry engine.
///
...
///
struct EnsembleConcept : RegisterEntryDescriptor<EnsembleType, EnsembleList> {
    ...
    ///
    static constexpr std::string_view entryName() { return ensembleName; }
    ...
    /// callback implementing the `ensemble` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    /// The `ensemble` concept does not define variant-level callbacks,
    ...
    ///
    /// This callback is invoked to determine whether the `ensemble`
