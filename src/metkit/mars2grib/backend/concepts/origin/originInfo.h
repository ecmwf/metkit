...
/**
 * @file originConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `origin` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `origin` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`OriginOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `origin` concept.
 *
 * `OriginConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `OriginType` variant.
 *
 * @note
...
struct OriginConceptInfo {
    ...
    /**
     * @brief Canonical name of the `origin` concept.
     *
     ...
     * - a pointer to the appropriate `OriginOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `OriginOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `originApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, OriginType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of an `origin` variant.
     *
     ...
     *
     * @tparam Variant Origin variant identifier
