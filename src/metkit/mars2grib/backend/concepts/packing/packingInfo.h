...
/**
 * @file packingConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `packing` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `packing` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`PackingOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `packing` concept.
 *
 * `PackingConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `PackingType` variant.
 *
 * @note
...
struct PackingConceptInfo {
     ...
     * Unlike most concepts, the name is defined explicitly as a string literal
     * rather than being derived from an enum header.
     */
     ...
     * - a pointer to the appropriate `PackingOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `PackingOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by
     * `packingApplicable<Stage, Section, Variant>()`.
     */
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t> entry() {
        if constexpr (packingApplicable<Stage, Section, Variant>()) {
            return &PackingOp<Stage, Section, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t>;
        }
        else {
            return nullptr;
        }
    ...
    /**
     * @brief Retrieve the human-readable name of a `packing` variant.
     *
     ...
     *
     * @tparam Variant Packing variant identifier
