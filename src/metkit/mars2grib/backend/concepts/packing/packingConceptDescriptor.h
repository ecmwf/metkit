...
///
/// @file packingConceptDescriptor.h
/// @brief Compile-time registry entry for the GRIB `packing` concept.
///
/// This header defines `PackingConcept`, the **compile-time descriptor**
/// that registers the GRIB `packing` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/packing/packingMatcher.h"
...
///
/// @brief Compile-time descriptor for the `packing` concept.
///
/// `PackingConcept` registers the GRIB `packing` concept into the
/// compile-time registry engine.
///
struct PackingConcept : RegisterEntryDescriptor<PackingType, PackingList> {
    ...
    ///
    static constexpr std::string_view entryName() { return packingName; }
    ...
    /// implementing the `packing` concept for a given:
    /// - capability
    /// - stage
    /// - GRIB section
    /// - variant
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, PackingType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (packingApplicable<Stage, Sec, Variant>()) {
                return &PackingOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    ///
    /// This hook is invoked to determine whether the `packing` concept should be
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &packingMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
