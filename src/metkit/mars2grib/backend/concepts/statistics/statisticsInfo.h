...
/**
 * @file statisticsConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `statistics` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `statistics` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`StatisticsOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `statistics` concept.
 *
 * `StatisticsConceptInfo` exposes the minimal interface required by the
...
struct StatisticsConceptInfo {
     ...
     * - a pointer to the appropriate `StatisticsOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `StatisticsOp`,
    ...
    /**
     * @brief Retrieve the human-readable name of a `statistics` variant.
     *
     ...
     *
     * @tparam Variant Statistics variant identifier
