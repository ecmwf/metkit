...
/**
 * @file dataTypeConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `data-type` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `data-type` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`DataTypeOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `data-type` concept.
 *
 * `DataTypeConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `DataTypeType` variant.
 *
 * @note
...
struct DataTypeConceptInfo {
    ...
    /**
     * @brief Canonical name of the `data-type` concept.
     *
     ...
     * - a pointer to the appropriate `DataTypeOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `DataTypeOp`, or `nullptr`
    ...
    /**
     * @brief Retrieve the human-readable name of a `data-type` variant.
     *
     ...
     *
     * @tparam Variant Data-type variant identifier
     * @return String view representing the variant name
     */
    template <auto Variant>
    static std::string_view variantName() {
        return std::string_view(dataTypeTypeName<static_cast<DataTypeType>(Variant)>());
    }
};
