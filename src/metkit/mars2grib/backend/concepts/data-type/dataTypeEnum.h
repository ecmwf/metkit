...
///
/// @file dataTypeEnum.h
/// @brief Definition of the `dataType` concept variants and compile-time metadata.
///
/// This header defines the **static description** of the GRIB `dataType` concept
/// used by the mars2grib backend. It contains:
///
/// - the canonical concept name (`dataTypeName`)
/// - the enumeration of supported data type variants (`DataTypeType`)
...
/// `dataType.h` / `dataTypeOp` implementation.
///
...
///
/// @brief Canonical name of the `dataType` concept.
///
/// This identifier is used:
/// - as the logical concept key in the concept registry
/// - for logging and debugging output
/// - to associate variants and capabilities with the `dataType` concept
///
...
///
/// @brief Enumeration of all supported `dataType` concept variants.
///
/// Each enumerator represents a specific data type or semantic interpretation
...
/// introduced in the future as the data type concept evolves.
///
/// @warning
...
///
/// @brief Compile-time list of all `dataType` concept variants.
///
...
///
/// @brief Compile-time mapping from `DataTypeType` to human-readable name.
///
/// This function returns the canonical string identifier associated
/// with a given data type variant.
///
...
#define DEF(T, NAME)                                   \
    template <>                                        \
    constexpr std::string_view dataTypeTypeName<T>() { \
        return NAME;                                   \
    }
