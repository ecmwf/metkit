...
/// that registers the GRIB `dataType` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/data-type/dataTypeMatcher.h"
...
///
/// @brief Compile-time descriptor for the `dataType` concept.
///
/// `DataTypeConcept` registers the GRIB `dataType` concept into the
/// compile-time registry engine.
///
...
///
struct DataTypeConcept : RegisterEntryDescriptor<DataTypeType, DataTypeList> {
    ...
    ///
    static constexpr std::string_view entryName() { return dataTypeName; }
    ...
    /// callback implementing the `dataType` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    /// The `dataType` concept does not define variant-level callbacks,
    ...
    ///
    /// This callback is invoked to determine whether the `dataType`
