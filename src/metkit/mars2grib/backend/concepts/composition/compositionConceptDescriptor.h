...
/// that registers the GRIB `composition` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/composition/compositionMatcher.h"
...
///
/// @brief Compile-time descriptor for the `composition` concept.
///
/// `CompositionConcept` registers the GRIB `composition` concept into the
/// compile-time registry engine.
///
...
///
struct CompositionConcept : RegisterEntryDescriptor<CompositionType, CompositionList> {
    ...
    ///
    static constexpr std::string_view entryName() { return compositionName; }
    ...
    /// callback implementing the `composition` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, CompositionType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (compositionApplicable<Stage, Sec, Variant>()) {
                return &CompositionOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    /// The `composition` concept does not define variant-level callbacks,
    ...
    ///
    template <std::size_t Capability, CompositionType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    ///
    /// This callback is invoked to determine whether the `composition`
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &compositionMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
