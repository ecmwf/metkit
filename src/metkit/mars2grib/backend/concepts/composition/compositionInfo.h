...
/**
 * @file compositionConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `composition` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `composition` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`CompositionOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `composition` concept.
 *
 * `CompositionConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `CompositionType` variant.
 *
 * @note
...
struct CompositionConceptInfo {
     ...
     * - a pointer to the appropriate `CompositionOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `CompositionOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `compositionApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, CompositionType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of a `composition` variant.
     *
     ...
     *
     * @tparam Variant Composition variant identifier
