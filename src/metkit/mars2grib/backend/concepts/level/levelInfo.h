...
/**
 * @file levelConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `level` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `level` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`LevelOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `level` concept.
 *
 * `LevelConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `LevelType` variant.
 *
 * @note
...
struct LevelConceptInfo {
     ...
     * - a pointer to the appropriate `LevelOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `LevelOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `levelApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, LevelType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
              class OutDict_t>
    ...
    /**
     * @brief Retrieve the human-readable name of a `level` variant.
     *
     ...
     *
     * @tparam Variant Level variant identifier
