...
/// that registers the GRIB `derived` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/derived/derivedMatcher.h"
...
///
/// @brief Compile-time descriptor for the `derived` concept.
///
/// `DerivedConcept` registers the GRIB `derived` concept into the
/// compile-time registry engine.
///
...
///
struct DerivedConcept : RegisterEntryDescriptor<DerivedType, DerivedList> {
    ...
    ///
    static constexpr std::string_view entryName() { return derivedName; }
    ...
    /// callback implementing the `derived` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, DerivedType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (derivedApplicable<Stage, Sec, Variant>()) {
                return &DerivedOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    /// The `derived` concept does not define variant-level callbacks,
    ...
    ///
    template <std::size_t Capability, DerivedType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    ///
    /// This callback is invoked to determine whether the `derived`
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &derivedMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
