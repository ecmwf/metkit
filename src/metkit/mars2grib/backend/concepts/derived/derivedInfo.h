...
/**
 * @file derivedConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `derived` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `derived` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`DerivedOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `derived` concept.
 *
 * `DerivedConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `DerivedType` variant.
 *
 * @note
 * This structure is intentionally stateless and fully constexpr-friendly.
 */
struct DerivedConceptInfo {
     ...
     */
    static constexpr const char* name = derivedName.data();
     ...
     * - a pointer to the appropriate `DerivedOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `DerivedOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `derivedApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, DerivedType Variant, class MarsDict_t, class GeoDict_t,
    ...
    static constexpr Fn<MarsDict_t, GeoDict_t, ParDict_t, OptDict_t, OutDict_t> entry() {
        if constexpr (derivedApplicable<Stage, Sec, Variant>()) {
            return &DerivedOp<Stage, Sec, Variant, MarsDict_t, GeoDict_t, ParDict_t, OptDict_t, OutDict_t>;
        }
        else {
            return nullptr;
        }
    ...
    /**
     * @brief Retrieve the human-readable name of a `derived` variant.
     *
     ...
     *
     * @tparam Variant Derived variant identifier
     * @return String view representing the variant name
     */
    template <auto Variant>
    static std::string_view variantName() {
        return std::string_view(derivedTypeName<static_cast<DerivedType>(Variant)>());
    }
};
