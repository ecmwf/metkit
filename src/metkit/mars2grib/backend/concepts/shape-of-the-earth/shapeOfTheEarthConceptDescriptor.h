...
///
/// @file shapeOfTheEarthConceptDescriptor.h
...
/// compile-time registry engine.
///
struct ShapeOfTheEarthConcept : RegisterEntryDescriptor<ShapeOfTheEarthType, ShapeOfTheEarthList> {
    ...
    ///
    static constexpr std::string_view entryName() { return shapeOfTheEarthName; }
    ...
    /// implementing the `shapeOfTheEarth` concept for a given:
    /// - capability
    /// - stage
    /// - GRIB section
    /// - variant
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, ShapeOfTheEarthType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (shapeOfTheEarthApplicable<Stage, Sec, Variant>()) {
                return &ShapeOfTheEarthOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    ///
    /// This hook is invoked to determine whether the `shapeOfTheEarth` concept should be
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &shapeOfTheEarthMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
