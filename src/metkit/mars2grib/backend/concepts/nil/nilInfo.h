...
/**
 * @file nilConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `nil` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `nil` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`NilOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `nil` concept.
 *
 * `NilConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `NilType` variant.
 *
 * @note
...
struct NilConceptInfo {
     ...
     * - a pointer to the appropriate `NilOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `NilOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `nilApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, NilType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
              class OutDict_t>
    ...
    /**
     * @brief Retrieve the human-readable name of a `nil` variant.
     *
     ...
     *
     * @tparam Variant Nil variant identifier
