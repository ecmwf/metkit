...
/// that registers the GRIB `generatingProcess` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/generating-process/generatingProcessMatcher.h"
...
///
/// @brief Compile-time descriptor for the `generatingProcess` concept.
///
/// `GeneratingProcessConcept` registers the GRIB `generatingProcess`
...
///
struct GeneratingProcessConcept : RegisterEntryDescriptor<GeneratingProcessType, GeneratingProcessList> {
    ...
    ///
    static constexpr std::string_view entryName() { return generatingProcessName; }
    ...
    static constexpr std::string_view variantName() {
        return generatingProcessTypeName<T>();
    }
    ...
    /// callback implementing the `generatingProcess` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, GeneratingProcessType Variant,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (generatingProcessApplicable<Stage, Sec, Variant>()) {
                return &GeneratingProcessOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    /// The `generatingProcess` concept does not define variant-level callbacks,
    ...
    ///
    template <std::size_t Capability, GeneratingProcessType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    ///
    /// This callback is invoked to determine whether the
    /// `generatingProcess` concept should be activated for a given
    /// encoding request.
    ///
    /// @tparam MarsDict_t Type of MARS dictionary
    /// @tparam OptDict_t  Type of options dictionary
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &generatingProcessMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
