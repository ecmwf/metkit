...
///
/// @file generatingProcessMatcher.h
/// @brief Entry-level matcher for the GRIB `generatingProcess` concept.
///
/// This header defines the runtime matcher used by the concept registry to
/// activate the default generating-process concept variant.
///
...
///
/// @brief Match the `generatingProcess` concept variant.
///
/// The generating-process concept is always active and resolves to
...
///
/// @return Local variant index for `GeneratingProcessType::Default`.
///
...
std::size_t generatingProcessMatcherImpl(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    utils::profiling::profileEnterFunction(cntx, Here());
    try {
        const std::size_t result = static_cast<std::size_t>(GeneratingProcessType::Default);
...
template <class MarsDict_t, class OptDict_t, class Cntx_t>
std::size_t generatingProcessMatcher(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    utils::profiling::profileEnterFunction(cntx, Here());
    const std::size_t result =
        generatingProcessMatcherImpl(mars, opt, utils::profiling::callSite(cntx, Here()));
    utils::profiling::profileExitFunction(cntx, Here());
    return result;
}
