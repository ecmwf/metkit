...
/**
 * @file generatingProcessConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `generating-process` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `generating-process` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`GeneratingProcessOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `generating-process` concept.
 *
 * `GeneratingProcessConceptInfo` exposes the minimal interface required by the
...
struct GeneratingProcessConceptInfo {
    ...
    /**
     * @brief Canonical name of the `generating-process` concept.
     *
     ...
     */
    static constexpr const char* name = generatingProcessName.data();
     ...
     * - a pointer to the appropriate `GeneratingProcessOp` specialization
     *   if the concept is applicable for the given parameters
     * - `nullptr` otherwise
     *
     ...
     *
     * @return Function pointer to the corresponding `GeneratingProcessOp`,
    ...
    /**
     * @brief Retrieve the human-readable name of a `generating-process` variant.
     *
     ...
     *
     * @tparam Variant Generating-process variant identifier
     * @return String view representing the variant name
     */
    template <auto Variant>
    static std::string_view variantName() {
        return std::string_view(generatingProcessTypeName<static_cast<GeneratingProcessType>(Variant)>());
    }
};
