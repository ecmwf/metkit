...
/**
 * @file paramConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `param` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `param` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`ParamOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `param` concept.
 *
 * `ParamConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `ParamType` variant.
 *
 * @note
...
struct ParamConceptInfo {
     ...
     * The name is defined explicitly as a string literal to match
     * the external MARS/GRIB naming convention.
     */
     ...
     * - a pointer to the appropriate `ParamOp` specialization if the concept
     ...
     * @tparam Variant    Param concept variant
     * @tparam MarsDict_t Type of the MARS dictionary
     * @tparam ParDict_t  Type of the parameter dictionary
     * @tparam OptDict_t  Type of the options dictionary
     * @tparam OutDict_t  Type of the output GRIB dictionary
     *
     * @return Function pointer to the corresponding `ParamOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `paramApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, ParamType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
              class OutDict_t>
    ...
    /**
     * @brief Retrieve the human-readable name of a `param` variant.
     *
     ...
     *
     * @tparam Variant Param variant identifier
