...
///
/// @file paramConceptDescriptor.h
/// @brief Compile-time registry entry for the GRIB `param` concept.
///
/// This header defines `ParamConcept`, the **compile-time descriptor**
/// that registers the GRIB `param` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/param/paramMatcher.h"
...
///
/// @brief Compile-time descriptor for the `param` concept.
///
/// `ParamConcept` registers the GRIB `param` concept into the
/// compile-time registry engine.
///
struct ParamConcept : RegisterEntryDescriptor<ParamType, ParamList> {
    ...
    ///
    static constexpr std::string_view entryName() { return paramName; }
    ...
    /// implementing the `param` concept for a given:
    /// - capability
    /// - stage
    /// - GRIB section
    /// - variant
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, ParamType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (paramApplicable<Stage, Sec, Variant>()) {
                return &ParamOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    ///
    /// This hook is invoked to determine whether the `param` concept should be
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &paramMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
