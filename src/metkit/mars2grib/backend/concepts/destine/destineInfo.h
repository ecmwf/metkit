...
/**
 * @file destineConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `destine` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `destine` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`DestineOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `destine` concept.
 *
 * `DestineConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `DestineType` variant.
 *
 * @note
...
struct DestineConceptInfo {
     ...
     * - a pointer to the appropriate `DestineOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `DestineOp`, or `nullptr`
     *         if the concept is not applicable.
     *
     * @note
     * Applicability is determined by `destineApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, DestineType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of a `destine` variant.
     *
     ...
     *
     * @tparam Variant Destine variant identifier
