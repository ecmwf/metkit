...
/// that registers the GRIB `destine` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/destine/destineMatcher.h"
...
///
/// @brief Compile-time descriptor for the `destine` concept.
///
/// `DestineConcept` registers the GRIB `destine` concept into the
/// compile-time registry engine.
///
...
///
struct DestineConcept : RegisterEntryDescriptor<DestineType, DestineList> {
    ...
    ///
    static constexpr std::string_view entryName() { return destineName; }
    ...
    /// callback implementing the `destine` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, DestineType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (destineApplicable<Stage, Sec, Variant>()) {
                return &DestineOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    /// The `destine` concept does not define variant-level callbacks,
    ...
    ///
    template <std::size_t Capability, DestineType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    ///
    /// This callback is invoked to determine whether the `destine`
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &destineMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
