...
/// that registers the GRIB `modelError` concept into the mars2grib
...
#include "metkit/mars2grib/backend/concepts/model-error/modelErrorEncoding.h"
#include "metkit/mars2grib/backend/concepts/model-error/modelErrorEnum.h"
#include "metkit/mars2grib/backend/concepts/model-error/modelErrorMatcher.h"
...
///
/// @brief Compile-time descriptor for the `modelError` concept.
///
/// `ModelErrorConcept` registers the GRIB `modelError` concept into the
/// compile-time registry engine.
///
...
///
struct ModelErrorConcept : RegisterEntryDescriptor<ModelErrorType, ModelErrorList> {
    ...
    ///
    static constexpr std::string_view entryName() { return modelErrorName; }
    ...
    /// callback implementing the `modelError` concept for a given:
    ///
    /// - Capability
    /// - Encoding stage
    /// - GRIB section
    ...
    ///
    template <std::size_t Capability, std::size_t Stage, std::size_t Sec, ModelErrorType Variant, class MarsDict_t,
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (modelErrorApplicable<Stage, Sec, Variant>()) {
                return &ModelErrorOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    ///
    template <std::size_t Capability, ModelErrorType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &modelErrorMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
