...
std::size_t modelErrorMatcherImpl(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    ...
    try {
        ...
        if (!has(mars, "type", utils::profiling::callSite(cntx, Here()))) {
        ...
        if ((get_or_throw<std::string>(mars, "type", utils::profiling::callSite(cntx, Here())) != "eme" &&
             get_or_throw<std::string>(mars, "type", utils::profiling::callSite(cntx, Here())) != "me")) {
            const std::size_t result = compile_time_registry_engine::MISSING;
            utils::profiling::profileExitFunction(cntx, Here());
            return result;
        }

        if (has(mars, "coeffindex", utils::profiling::callSite(cntx, Here()))) {
            const std::size_t result = static_cast<std::size_t>(ModelErrorType::FourierCoefficients);
            utils::profiling::profileExitFunction(cntx, Here());
            return result;
        }
        ...
        if (has(mars, "number", utils::profiling::callSite(cntx, Here()))) {
            const std::size_t result = static_cast<std::size_t>(ModelErrorType::ComponentIndex);
            utils::profiling::profileExitFunction(cntx, Here());
            return result;
        }
...
template <class MarsDict_t, class OptDict_t, class Cntx_t>
std::size_t modelErrorMatcher(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    utils::profiling::profileEnterFunction(cntx, Here());
    const std::size_t result = modelErrorMatcherImpl(mars, opt, utils::profiling::callSite(cntx, Here()));
    utils::profiling::profileExitFunction(cntx, Here());
    return result;
}
