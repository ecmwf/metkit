...
/**
 * @file satelliteConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `satellite` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `satellite` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`SatelliteOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `satellite` concept.
 *
 * `SatelliteConceptInfo` exposes the minimal interface required by the
...
struct SatelliteConceptInfo {
     ...
     * - a pointer to the appropriate `SatelliteOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `SatelliteOp`,
    ...
    /**
     * @brief Retrieve the human-readable name of a `satellite` variant.
     *
     ...
     *
     * @tparam Variant Satellite variant identifier
