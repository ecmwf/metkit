...
/**
 * @file tablesConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `tables` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `tables` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`TablesOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `tables` concept.
 *
 * `TablesConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `TablesType` variant.
 *
 * @note
...
struct TablesConceptInfo {
     ...
     * - a pointer to the appropriate `TablesOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `TablesOp`,
     *         or `nullptr` if the concept is not applicable.
     *
     * @note
     * Applicability is determined by
     * `tablesApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, TablesType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of a `tables` variant.
     *
     ...
     *
     * @tparam Variant Tables variant identifier
