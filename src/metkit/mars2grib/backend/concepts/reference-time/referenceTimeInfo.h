...
/**
 * @file referenceTimeConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `reference-time` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `reference-time` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`ReferenceTimeOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `reference-time` concept.
 *
 * `ReferenceTimeConceptInfo` exposes the minimal interface required by the
...
struct ReferenceTimeConceptInfo {
    ...
    /**
     * @brief Canonical name of the `reference-time` concept.
     *
     ...
     * - a pointer to the appropriate `ReferenceTimeOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `ReferenceTimeOp`,
     *         or `nullptr` if the concept is not applicable.
     *
     * @note
     * Applicability is determined by
     * `referenceTimeApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, ReferenceTimeType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of a `reference-time` variant.
     *
     ...
     *
     * @tparam Variant Reference-time variant identifier
