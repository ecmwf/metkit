...
/**
 * @file waveConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `wave` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `wave` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`WaveOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `wave` concept.
 *
 * `WaveConceptInfo` exposes the minimal interface required by the
 ...
 * **[Stage × Section] function table** for each supported `WaveType` variant.
 *
 * @note
...
struct WaveConceptInfo {
     ...
     * - a pointer to the appropriate `WaveOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `WaveOp`,
     *         or `nullptr` if the concept is not applicable.
     *
     * @note
     * Applicability is determined by
     * `waveApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, WaveType Variant, class MarsDict_t, class ParDict_t, class OptDict_t,
              class OutDict_t>
    ...
    /**
     * @brief Retrieve the human-readable name of a `wave` variant.
     *
     ...
     *
     * @tparam Variant Wave variant identifier
