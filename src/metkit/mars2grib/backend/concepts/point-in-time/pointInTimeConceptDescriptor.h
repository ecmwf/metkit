...
///
/// @file pointInTimeConceptDescriptor.h
...
struct PointInTimeConcept : RegisterEntryDescriptor<PointInTimeType, PointInTimeList> {
    ...
    /// implementing the `pointInTime` concept for a given:
    /// - capability
    /// - stage
    /// - GRIB section
    /// - variant
    ...
    static constexpr Fn<MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t> phaseCallbacks() {
        ...
        if constexpr (Capability == 0) {
            ...
            if constexpr (pointInTimeApplicable<Stage, Sec, Variant>()) {
                return &PointInTimeOp<Stage, Sec, Variant, MarsDict_t, ParDict_t, OptDict_t, OutDict_t, Cntx_t>;
            }
            else {
                return nullptr;
            }
    ...
    ///
    /// This hook is invoked to determine whether the `pointInTime` concept should be
    ...
    static constexpr Fm<MarsDict_t, OptDict_t, Cntx_t> entryCallbacks() {
        if constexpr (Capability == 0) {
            return &pointInTimeMatcher<MarsDict_t, OptDict_t, Cntx_t>;
        }
        else {
            return nullptr;
        }
