...
/**
 * @file pointInTimeConceptInfo.h
 * @brief ConceptInfo definition for the GRIB `point-in-time` concept.
 *
 * This header defines the **ConceptInfo** structure associated with the
 * `point-in-time` concept in the mars2grib backend.
 *
 * A ConceptInfo acts as the **compile-time glue** between:
 * - a concept name
 * - its set of variants
 * - the applicability rules
 * - the concrete encoding operation (`PointInTimeOp`)
 *
...
/**
 * @brief Compile-time metadata and entry-point provider for the `point-in-time` concept.
 *
 * `PointInTimeConceptInfo` exposes the minimal interface required by the
...
struct PointInTimeConceptInfo {
    ...
    /**
     * @brief Canonical name of the `point-in-time` concept.
     *
     ...
     * - a pointer to the appropriate `PointInTimeOp` specialization if the concept
     ...
     *
     * @return Function pointer to the corresponding `PointInTimeOp`,
     *         or `nullptr` if the concept is not applicable.
     *
     * @note
     * Applicability is determined by
     * `pointInTimeApplicable<Stage, Sec, Variant>()`.
     */
    template <std::size_t Stage, std::size_t Sec, PointInTimeType Variant, class MarsDict_t, class ParDict_t,
    ...
    /**
     * @brief Retrieve the human-readable name of a `point-in-time` variant.
     *
     ...
     *
     * @tparam Variant Point-in-time variant identifier
