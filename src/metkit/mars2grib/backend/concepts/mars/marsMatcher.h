...
///
/// @file marsMatcher.h
/// @brief Entry-level matcher for the GRIB `mars` concept.
///
/// This header defines the runtime matcher used by the concept registry to
/// decide whether MARS provenance metadata is active for a request.
///
...
///
/// @brief Match the `mars` concept variant.
///
/// The concept is active as `MarsType::Default` when `class`, `type`, `stream`,
/// and `expver` are present in the MARS request.
///
...
///
/// @return Local variant index for `MarsType::Default`, or
...
std::size_t marsMatcherImpl(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    utils::profiling::profileEnterFunction(cntx, Here());
    try {
        using metkit::mars2grib::utils::dict_traits::has;

        if (has(mars, "class", utils::profiling::callSite(cntx, Here())) && has(mars, "type", utils::profiling::callSite(cntx, Here())) && has(mars, "stream", utils::profiling::callSite(cntx, Here())) &&
            has(mars, "expver", utils::profiling::callSite(cntx, Here()))) {
            const std::size_t result = static_cast<size_t>(MarsType::Default);
...
template <class MarsDict_t, class OptDict_t, class Cntx_t>
std::size_t marsMatcher(const MarsDict_t& mars, const OptDict_t& opt, Cntx_t& cntx) {
    utils::profiling::profileEnterFunction(cntx, Here());
    const std::size_t result = marsMatcherImpl(mars, opt, utils::profiling::callSite(cntx, Here()));
    utils::profiling::profileExitFunction(cntx, Here());
    return result;
}
