...
inline bool isSupportedTimespanString(const std::string& value) {
    constexpr std::array<std::string_view, 12> supported{
        "1h", "3h", "6h", "12h", "18h", "24h", "48h", "72h", "120h", "168h", "240h", "360h"};
...
inline bool allowNonEnumeratedPositiveIntegerTimespanHours(const OptDict_t& opt) {
    ...
    throw Mars2GribDeductionException("Invalid boolean option `allowNonEnumeratedPositiveIntegerTimespanHours`", Here());
}
...
std::optional<Timespan> resolve_Timespan_opt(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        if (auto value = get_opt<std::string>(mars, "timespan")) {
            ...
            if (normalized == "none") {
                ...
                if (detail::isRecognizedUnsupportedTimespan(normalized)) {
                    throw Mars2GribDeductionException("Recognized but unsupported `timespan`: '" + *value + "'", Here());
                }
            ...
            if (!detail::allowNonEnumeratedPositiveIntegerTimespanHours(opt) && !detail::isSupportedTimespanHours(*value)) {
                throw Mars2GribDeductionException(
        ...
        std::throw_with_nested(Mars2GribDeductionException(
            "Failed to resolve `timespan` from input dictionaries", Here()));
    }
...
Timespan resolve_Timespan_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        throw Mars2GribDeductionException("`timespan` is not defined in the Mars dictionary", Here());
    } catch (...) {
