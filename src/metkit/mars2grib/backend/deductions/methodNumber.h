...
/**
 * @brief Resolve the wave processing method identifier from the MARS dictionary.
 *
 * This deduction retrieves the value associated with the key `method`
 ...
 *
 * @tparam MarsDict_t
 *   Type of the MARS dictionary, expected to contain the key `method`.
 *
 * @tparam ParDict_t
 ...
 *
 * @return
 *   The method identifier resolved from the MARS dictionary, returned
 *   as a `long`.
 *
 * @throws metkit::mars2grib::utils::exceptions::Mars2GribDeductionException
 *   If:
 *   - the key `method` is not present in the MARS dictionary,
...
long resolve_MethodNumber_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        // Get the mars.method
        auto methodNumber = get_or_throw<long>(mars, "method");

        // Logging of the methodNumber
        MARS2GRIB_LOG_RESOLVE([&]() {
            std::string logMsg = "methodNumber: looked up from Mars dictionary with value: ";
        ...
        std::throw_with_nested(Mars2GribDeductionException("Unable to get `method` from Mars dictionary", Here()));
    };
