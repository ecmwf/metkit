...
std::optional<TimeDuration> resolve_TimeIncrement_opt(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        MARS2GRIB_LOG_RESOLVE([&]() {
            return std::string{"`timeIncrement` resolved from input dictionaries: value='"} + std::to_string(result.length) +
                   "' unit='" + tables::enum2name_TimeUnit_or_throw(result.unit) + "'";
        }());
        return result;
    } catch (...) {
        std::throw_with_nested(Mars2GribDeductionException(
            "Failed to resolve `timeIncrement` from input dictionaries", Here()));
    }
...
TimeDuration resolve_TimeIncrement_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        std::throw_with_nested(Mars2GribDeductionException(
            "Unable to get `timeIncrement` from Par dictionary", Here()));
    }
