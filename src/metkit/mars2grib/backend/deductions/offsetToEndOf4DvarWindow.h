 ...
 *
 * This deduction retrieves the value associated with the key `anoffset`
 * from the MARS dictionary (`mars`). The value is expected to be convertible
 * to a `long` and is treated as mandatory.
 *
 * The resolved value represents the temporal offset (typically expressed in
 * minutes or hours, depending on upstream conventions) from the reference
 * analysis time to the end of the 4D-Var assimilation window. The precise
 * unit and interpretation of this offset are defined by the MARS/IFS
 * conventions and are not interpreted by this deduction.
 *
 ...
 *
 * @tparam MarsDict_t
 *   Type of the MARS dictionary, expected to contain the key `anoffset`.
 *
 * @tparam ParDict_t
 ...
 *
 * @param[in] mars
 *   MARS dictionary from which the offset to the end of the 4D-Var window
 *   is retrieved.
 *
 * @param[in] par
 ...
 *
 * @return
 *   The offset to the end of the 4D-Var analysis window resolved from the
 *   MARS dictionary, returned as a `long`.
 *
 * @throws metkit::mars2grib::utils::exceptions::Mars2GribDeductionException
 *   If:
 *   - the key `anoffset` is not present in the MARS dictionary,
 *   - the associated value cannot be converted to `long`,
 *   - any unexpected error occurs during dictionary access.
 *
 * @note
 *   This deduction assumes that the offset to the end of the 4D-Var window
 *   is explicitly provided by the MARS dictionary and does not attempt any
 *   inference, defaulting, or validation of the temporal semantics.
 *
 * @note
...
long resolve_offsetToEndOf4DvarWindow_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        // Logging of the offsetToEndOf4DvarWindow
        MARS2GRIB_LOG_RESOLVE([&]() {
            std::string logMsg = "offsetToEndOf4DvarWindow: looked up from Mars dictionary with value: ";
