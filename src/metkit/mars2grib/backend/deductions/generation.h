...
long resolve_Generation_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt, Cntx_t& cntx) {
    ...
    try {
        ...
        // Retrieve mandatory MARS generation
        long marsGenerationVal = get_or_throw<long>(mars, "generation", metkit::mars2grib::utils::profiling::callSite(cntx, Here()));

        // Emit RESOLVE log entry
        MARS2GRIB_LOG_RESOLVE([&]() {
            std::string logMsg =
                "`generation` resolved from input dictionaries: value='" + std::to_string(marsGenerationVal) + "'";
            return logMsg;
        }());

        // Success exit point
        {
            long result = marsGenerationVal;
