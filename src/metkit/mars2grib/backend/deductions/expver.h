...
std::string resolve_Expver_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt, Cntx_t& cntx) {
    ...
    try {
        ...
        std::string marsExpverVal = get_or_throw<std::string>(mars, "expver", metkit::mars2grib::utils::profiling::callSite(cntx, Here()));
        ...
        // Success exit point
        {
            std::string result = marsExpverVal;
