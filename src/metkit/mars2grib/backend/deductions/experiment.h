...
std::string resolve_Experiment_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt, Cntx_t& cntx) {
    ...
    try {
        ...
        std::string marsExperimentVal = get_or_throw<std::string>(mars, "experiment", metkit::mars2grib::utils::profiling::callSite(cntx, Here()));
        ...
        // Success exit point
        {
            std::string result = marsExperimentVal;
