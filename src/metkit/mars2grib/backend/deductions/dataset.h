...
/**
 * @brief Resolve the dataset identifier from the MARS dictionary.
 *
 * This deduction retrieves the value associated with the key `dataset`
 * from the MARS dictionary (`mars`). The value is expected to be a
 * string and is treated as mandatory.
 *
 * The resolved value typically identifies the dataset from which the
 * data originates (e.g. reanalysis, operational archive, experimental
 * dataset). The semantics and allowed values of the dataset identifier
 * are defined by upstream MARS conventions and are not interpreted by
 * this deduction.
 *
 ...
 *
 * @tparam MarsDict_t
 *   Type of the MARS dictionary, expected to contain the key `dataset`.
 *
 * @tparam ParDict_t
 ...
 *
 * @param[in] mars
 *   MARS dictionary from which the dataset identifier is retrieved.
 *
 * @param[in] par
 ...
 *
 * @return
 *   The dataset identifier resolved from the MARS dictionary,
 *   returned as a `std::string`.
 *
 * @throws metkit::mars2grib::utils::exceptions::Mars2GribDeductionException
 *   If:
 *   - the key `dataset` is not present in the MARS dictionary,
 *   - the associated value cannot be converted to `std::string`,
 *   - any unexpected error occurs during dictionary access.
 *
 * @note
 *   This deduction assumes that the dataset identifier is explicitly
...
std::string resolve_Dataset_or_throw(const MarsDict_t& mars, const ParDict_t& par, const OptDict_t& opt) {
    ...
    try {
        ...
        // Logging of the dataset
        MARS2GRIB_LOG_RESOLVE([&]() {
            std::string logMsg = "dataset: deduced from mars dictionary with value: " + marsDatasetVal;
            return logMsg;
        }());
