...
constexpr Fm<MarsDict_t, OptDict_t> makeEntryCallback() {
    return Entry::template entryCallbacks<
        Capability,
        MarsDict_t,
        OptDict_t
    >();
...
struct BuildEntryCallbacks<
    ...
    OptDict_t
> {
    static constexpr std::array<Fm<MarsDict_t, OptDict_t>, 0> value() {
        return {};
    }
};
...
> {
    static constexpr auto value() {
        constexpr auto head =
            std::array<Fm<MarsDict_t, OptDict_t>, 1>{
                makeEntryCallback<Head, Capability, MarsDict_t, OptDict_t>()
            };
...
constexpr auto makeEntryCallbacksRegistry() {
    return detail::BuildEntryCallbacks<
        EntriesList,
        Capability,
        MarsDict_t,
        OptDict_t
