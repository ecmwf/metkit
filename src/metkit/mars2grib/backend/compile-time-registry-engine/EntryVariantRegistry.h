...
constexpr std::size_t value_index_impl(Enum v, std::size_t pos, ValueList<Head, Tail...>) {
    return (v == static_cast<Enum>(Head))
               ? pos
               : value_index_impl<Enum>(v, pos + 1, ValueList<Tail...>{});
}
...
constexpr std::array<std::size_t, Concept::variant_count>
concept_id_block_impl(std::index_sequence<Is...>) {
...
constexpr std::array<std::size_t, Concept::variant_count>
concept_id_block() {
    return concept_id_block_impl<ConceptIndex, Concept>(
        std::make_index_sequence<Concept::variant_count>{});
}

template <typename Concept, std::size_t... Is>
constexpr std::array<std::size_t, Concept::variant_count>
variant_id_block_impl(std::index_sequence<Is...>) {
    return {{Is...}};
}
...
template <typename Concept, std::size_t... Is>
constexpr std::array<std::string_view, Concept::variant_count>
concept_name_block_impl(std::index_sequence<Is...>) {
...
constexpr std::array<std::string_view, sizeof...(Vs)>
variant_name_block_impl(ValueList<Vs...>) {
...
struct BuildConceptIdTableWithBase<TypeList<Head, Tail...>, BaseIndex> {
    static constexpr auto value() {
        return concat(
            concept_id_block<BaseIndex, Head>(),
...
struct BuildConceptNames<TypeList<Head, Tail...>> {
    static constexpr auto value() {
        return concat(
            std::array<std::string_view, 1>{{Head::entryName()}},
...
private:
    template <typename Concept, std::size_t... Is>
    static constexpr std::array<std::size_t, Concept::variant_count>
    make_id_array_from_concept_impl(std::index_sequence<Is...>) {
...
public:
    template <typename Concept>
    static constexpr std::array<std::size_t, Concept::variant_count>
    make_id_array_from_concept() {
        return make_id_array_from_concept_impl<Concept>(std::make_index_sequence<Concept::variant_count>{});
    }

    template <typename Concept, auto... Vs>
    static constexpr std::array<std::size_t, sizeof...(Vs)>
    make_id_array_from_variants() {
    ...
    static std::size_t localIndex(const std::string& conceptName,
                                  const std::string& variantName) {
        const std::size_t cid = conceptId(conceptName);
        if (cid == missing) return missing;

        const std::size_t begin = offset(cid);
        const std::size_t end =
            (cid + 1 < NConcepts) ? offset(cid + 1) : NVariants;
    ...
    static std::size_t globalIndex(const std::string& conceptName,
                                   const std::string& variantName) {
        const std::size_t cid = conceptId(conceptName);
        if (cid == missing) return missing;
