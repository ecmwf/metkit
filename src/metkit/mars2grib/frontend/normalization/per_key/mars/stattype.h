...
///
/// @brief Sanitize the MARS key: stattype.
///
...
void sanitise_stattype_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    ...
    if (auto raw = get_opt<std::string>(in, "stattype")) {
        set_or_throw<std::string>(out, "stattype",
                                  enum_helper::resolve_canonical(language, "stattype", *raw));
    }
}
