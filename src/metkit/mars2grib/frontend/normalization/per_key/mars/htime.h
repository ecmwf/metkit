...
///
/// @brief Sanitize the MARS key: htime (hindcast time, HHMM).
///
/// No-op if absent (backend defaults to 0 when missing).
///
...
void sanitise_htime_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    ...
    if (auto v = get_opt<long>(in, "htime")) {
        set_or_throw<long>(out, "htime", *v);
    }
}
