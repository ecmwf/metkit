...
///
/// @brief Sanitize the MARS key: stream.
///
...
void sanitise_stream_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    using metkit::mars2grib::utils::dict_traits::get_opt;
    using metkit::mars2grib::utils::dict_traits::set_or_throw;
    if (auto raw = get_opt<std::string>(in, "stream")) {
        set_or_throw<std::string>(out, "stream",
                                  enum_helper::resolve_canonical(language, "stream", *raw));
    }
}
