...
///
/// @brief Sanitize the MARS key: resolution.
///
...
void sanitise_resolution_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    ...
    if (auto raw = get_opt<std::string>(in, "resolution")) {
        set_or_throw<std::string>(out, "resolution",
                                  enum_helper::resolve_canonical(language, "resolution", *raw));
    }
}
