...
///
/// @brief Sanitize the MARS key: activity.
///
...
void sanitise_activity_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    using metkit::mars2grib::utils::dict_traits::get_opt;
    using metkit::mars2grib::utils::dict_traits::set_or_throw;
    if (auto raw = get_opt<std::string>(in, "activity")) {
        set_or_throw<std::string>(out, "activity",
                                  enum_helper::resolve_canonical(language, "activity", *raw));
    }
}
