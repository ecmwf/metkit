...
///
/// @brief Sanitize the MARS key: experiment.
///
...
void sanitise_experiment_or_throw(const MarsDict_t& in, MarsDict_t& out, const eckit::Value& language) {
    ...
    if (auto raw = get_opt<std::string>(in, "experiment")) {
        set_or_throw<std::string>(out, "experiment",
                                  enum_helper::resolve_canonical(language, "experiment", *raw));
    }
}
