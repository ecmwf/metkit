...
using EncodingRegistry = metkit::mars2grib::backend::concepts_::EncodingCallbacksRegistry<MarsDict_t, ParDict_t, OptDict_t, OutDict_t>;
 ...
 */
template <class MarsDict_t, class ParDict_t, class OptDict_t, class OutDict_t>
using EncodingPlan = std::array<
    std::array<
        FixedVector<Fn_t<MarsDict_t, ParDict_t, OptDict_t, OutDict_t>,GeneralRegistry::NConcepts>,
 ...
 */
template <class MarsDict_t, class ParDict_t, class OptDict_t, class OutDict_t>
EncodingPlan<MarsDict_t, ParDict_t, OptDict_t, OutDict_t>
make_EncodingPlan_or_throw(const GribHeaderLayoutData& headerLayout) {
    ...
    try {
        ...
        for (std::size_t sid = 0; sid < GeneralRegistry::NSections; ++sid) {
            ...
            table[0][sid].push_back( metkit::mars2grib::backend::sections::initializers::sectionRegistry<MarsDict_t, ParDict_t, OptDict_t, OutDict_t>(sid, templ) );
        }
        ...
        std::throw_with_nested(
            Mars2GribGenericException("Unable to create encoding plan", Here())
        );
    }
}
