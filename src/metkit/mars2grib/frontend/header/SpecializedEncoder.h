...
public:
     ...
     */
    using Plan_t = metkit::mars2grib::frontend::header::detail::EncodingPlan<MarsDict_t, ParDict_t, OptDict_t, OutDict_t>;
     ...
     */
    std::unique_ptr<OutDict_t> encode(const MarsDict_t& mars, const ParDict_t& par,
        ...
        try {
            ...
            std::throw_with_nested(Mars2GribEncoderException(
                ...
                dict_to_json<MarsDict_t>(mars),
                dict_to_json<ParDict_t>(par),
                dict_to_json<OptDict_t>(opt),
                debug_convert_GribHeaderLayoutData_to_json(layout_),
                Here()));
        }
    }
