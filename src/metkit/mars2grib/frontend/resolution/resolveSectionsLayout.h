 ...
 */
inline std::array<SectionLayoutData, backend::concepts_::GeneralRegistry::NSections>
resolve_SectionsLayout_or_throw(const ActiveConceptsData& activeConcepts) {
        ...
        for (std::size_t section = 0; section < GeneralRegistry::NSections; ++section) {
            ...
            if (sectionData.sectionNumber != section) {
                throw Mars2GribGenericException(
                    "SectionTemplateSelector mismatch: expected section " + std::to_string(section) +
                    " but recipe returned " + std::to_string(sectionData.sectionNumber), Here());
            }
...
inline void debug_print_resolved_layout(
    ...
    for(const auto& s : layout) {
        os << "Section " << s.sectionNumber << " -> Template " << s.templateNumber
           << " (Concepts: " << s.count << ")" << std::endl;
    }
}
