...
static long readcb(void *data, void *buffer, long len) {
  ...
  try {
    ...
    eckit::Log::error()
        << "Exception thrown in CodesSplitter::readcb callback: " << e.what()
        << "." << std::endl
        << " This may cause unexpected behaviour. Returning -2 instead."
        << std::endl;
    // Return negative other from -1 to signalize error.
    return -2;
  } catch (...) {
    eckit::Log::error() << "Unknown exception occured in CodesSplitter::readcb "
                           "callback. This may cause unexpected "
...
eckit::message::Message CodesSplitter::next() {
  ...
  if (err != 0 and err != GRIB_END_OF_FILE) {
    ...
    if (err == GRIB_WRONG_LENGTH && handle_.canSeek()) {
      ...
      handle_.seek((off < eckit::Offset(0) ? eckit::Offset(0) : off) +
                   eckit::Offset(4));
    }
    CODES_CALL(err);
  }
...
void CodesSplitter::print(std::ostream &s) const { s << "CodesSplitter[]"; }
...
template <>
bool SplitterBuilder<metkit::codes::CodesSplitter>::match(
    eckit::PeekHandle &handle) const {

  constexpr std::array<std::array<char, 4>, 4> valid_tags{
      {{'G', 'R', 'I', 'B'},
       {'B', 'U', 'F', 'R'},
       {'B', 'U', 'D', 'G'},
       {'T', 'I', 'D', 'E'}}};

  if (std::array<char, 4> tag; handle.peek(tag.data(), 4) == 4) {
    for (const auto &valid_tag : valid_tags) {
      if (tag == valid_tag) {
        return true;
      }
    }
    // TODO(kkratz): This should probaly trigger an error
  };
