...
/// Concrete implementation of CodesHandle. CodesHandleRef is a non-owning container around a CodesHandlePtr* that
/// makes the C APi accessible to C++ for any CodesHandlePtr*
/// \see OwningCodesHandle
class CodesHandleRef : public CodesHandle {
public:  // methods

    CodesHandleRef(CodesHandlePtr*);

    CodesHandleRef(const CodesHandleRef&)            = default;
    CodesHandleRef(CodesHandleRef&&)                 = default;
    CodesHandleRef& operator=(const CodesHandleRef&) = default;
    CodesHandleRef& operator=(CodesHandleRef&&)      = default;
    ...
    bool isDefined(const std::string&) const override;

    /// Check if a key is missing
    bool isMissing(const std::string&) const override;
    ...
    void set(const std::string& key, Span<const char*>) override;              /// set string array
    void set(const std::string& key, Span<const double>) override;
