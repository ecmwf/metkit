...
/// @brief Read-only dictionary-access traits for Grib2Mars Options.
///
/// This adapter exposes the strongly typed `Options` aggregate through the
/// generic Grib2Mars dictionary API.
///
...
///
template <>
constexpr std::string_view type_name<metkit::grib2mars::Options>() {
    return "metkit::grib2mars::Options";
}
...
inline bool getBoolOrThrow(const Options& opts, std::string_view key) {
    ...
    throw exceptions::Grib2MarsDictException("Key `" + std::string(key) + "` cannot be read as `" + "bool" +
                                                 "` from dictionary type `metkit::grib2mars::Options`",
                                             Here());
}
...
inline std::string getStringOrThrow(const Options& opts, std::string_view key) {
    ...
    throw exceptions::Grib2MarsDictException("Key `" + std::string(key) + "` cannot be read as `" + "std::string" +
                                                 "` from dictionary type `metkit::grib2mars::Options`",
                                             Here());
}
...
struct DictToJsonTraits<Options> {
    ...
    static std::string to_json(const Options& opts) noexcept(true) {
        try {
            ...
            return "[to_json failed for metkit::grib2mars::Options]";
        }
    }
};
