...
inline const std::string& scalarStringOrThrow(const metkit::mars::MarsRequest& mars, std::string_view key) {
    ...
    if (!mars.has(k)) {
        throw exceptions::Grib2MarsDictException("Missing key `"s + k + "` in metkit::mars::MarsRequest", Here());
    }
    ...
    if (values.size() != 1) {
        throw exceptions::Grib2MarsDictException(
            "Invalid non-scalar key `"s + k + "` in metkit::mars::MarsRequest: found `"s +
                std::to_string(values.size()) + "` values. grib2mars expects a single MARS point, not a hypercube.",
            Here());
    }
...
inline long parseHoursAsLong(std::string_view value, std::string_view key) {
    ...
    if (s.empty()) {
        throw exceptions::Grib2MarsDictException("Cannot convert empty key `"s + std::string{key} + "` value to hours",
                                                 Here());
    }
    ...
    if (s.back() == 'm' || s.back() == 's') {
        throw exceptions::Grib2MarsDictException(
            "Cannot convert key `"s + std::string{key} + "` value `"s + s +
                "` to hours: sub-hourly extents are not supported by this grib2mars path",
            Here());
    }
...
inline void checkYYYYMMDD(long yyyymmdd, std::string_view key) {
    ...
    if (year <= 0 || month < 1 || month > 12 || day < 1 || day > daysInMonth(year, month)) {
        throw exceptions::Grib2MarsDictException("Invalid MARS date `"s + std::to_string(yyyymmdd) + "` for key `"s +
...
inline std::string longToMarsDate(long value) {
    ...
    if (s.size() != 8 || !allDigits(s)) {
        throw exceptions::Grib2MarsDictException("Cannot convert integer `"s + s + "` to MARS date. Expected yyyymmdd.",
                                                 Here());
    }
...
inline long makeHHMMSS(long hh, long mm, long ss, std::string_view value, std::string_view key) {
    if (hh < 0 || hh > 23 || mm < 0 || mm > 59 || ss < 0 || ss > 59) {
        throw exceptions::Grib2MarsDictException("Invalid MARS time `"s + std::string{value} + "` for key `"s +
