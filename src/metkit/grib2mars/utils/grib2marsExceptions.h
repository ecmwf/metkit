...
/// @file grib2marsExceptions.h
/// @brief Unified exception hierarchy for the grib2mars framework.
///
...
/// @brief Base exception for grib2mars.
///
/// This is the root exception type for most grib2mars failures.
/// It:
///
...
class Grib2MarsGenericException : public eckit::Exception, public std::nested_exception {
public:

    Grib2MarsGenericException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        eckit::Exception(reason, loc) {}

    virtual ~Grib2MarsGenericException() = default;
...
class Grib2MarsRulesException : public eckit::Exception, public std::nested_exception {
public:

    Grib2MarsRulesException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        eckit::Exception(reason, loc) {}
};
...
class Grib2MarsDictException : public Grib2MarsGenericException {
public:

    Grib2MarsDictException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Grib2MarsGenericException(reason, loc) {}
};
...
///
/// This function detects `Grib2MarsGenericException`
...
inline void printExtendedStack(const std::exception& e, std::ostream& os, std::size_t level = 0,
    ...
    if (const auto* me = dynamic_cast<const Grib2MarsGenericException*>(&e)) {
