...
void extractTime(const std::string& keyword,
    ...
    try {
        ...
        if (!grib.has("hour")) {
            throw Grib2MarsGenericException(
                "Missing GRIB key `hour` required to extract MARS keyword `" +
                    keyword + "`",
                Here());
        }
        ...
        if (!grib.has("minute")) {
            throw Grib2MarsGenericException(
                "Missing GRIB key `minute` required to extract MARS keyword `" +
                    keyword + "`",
                Here());
        }
        ...
        if (!grib.has("second")) {
            throw Grib2MarsGenericException(
                "Missing GRIB key `second` required to extract MARS keyword `" +
                    keyword + "`",
                Here());
        }
