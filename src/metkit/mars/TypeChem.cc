...
#include "metkit/mars/TypeChem.h"
...
static void init() {
    ...
    const eckit::Value ids = eckit::YAMLParser::decodeFile(LibMetkit::chemIDYamlFile());
...
TypeChem::TypeChem(const std::string& name, const eckit::Value& settings) : Type(name, settings), firstRule_(false) {
...
void TypeChem::print(std::ostream& out) const {
    out << "TypeChem[name=" << name_ << "]";
}

void TypeChem::pass2(const MarsExpandContext& ctx, MarsRequest& request) {
    ...
    if (!rule) {
        Log::warning() << "TypeChem: cannot find a context to expand 'param' in " << request << std::endl;
        ...
        if (firstRule_) {
                ...
                if ((*j).match(request, true)) {
                    for (std::vector<std::string>::iterator j = values.begin(); j != values.end() && !rule; ++j) {
                        ...
                        try {
                            ...
                            rule = r;
                            Log::warning() << "TypeChem: using 'first matching rule' option " << *rule << std::endl;
                        }
                        catch (...) {
                        }
                    }
            ...
            for (std::vector<Rule>::const_iterator j = rules->begin(); j != rules->end(); ++j) {
                if ((*j).match(tmp)) {
                    rule = &(*j);
                    Log::warning() << "TypeChem using 'expand with' option " << *rule << std::endl;
                    break;
                }
            }
        }
        if (!rule) {
            std::ostringstream oss;
            oss << "TypeChem: cannot find a context to expand 'param' in " << request;
...
bool TypeChem::expand(const MarsExpandContext&, std::string&, const MarsRequest&) const {
    // Work done on pass2()
    return true;
}
...
static TypeBuilder<TypeChem> type("chem");
