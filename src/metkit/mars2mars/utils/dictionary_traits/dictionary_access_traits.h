...
inline T get_or_throw(const Dict& dict, std::string_view key) {
    try {
        ...
        std::throw_with_nested(
            exceptions::Mars2marsDictException("Forwarding errors while getting key `"s + std::string(key) + "` as `" +
...
inline void set_or_throw(Dict& dict, std::string_view key, const T& value) {
    try {
        ...
        std::throw_with_nested(exceptions::Mars2marsDictException("Forwarding errors while setting key `"s +
                                                                      std::string(key) + "` as `" +
