...
/// diagnostic context.
class Mars2marsMatcherException : public Mars2marsGenericException {
public:

    Mars2marsMatcherException(long param, const std::string& levtype, std::string reason,
                              const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc), param_{std::to_string(param)}, levtype_{levtype} {}

    Mars2marsMatcherException(const std::string& levtype, std::string reason,
                              const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc), param_{std::nullopt}, levtype_{levtype} {}

    Mars2marsMatcherException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc), param_{std::nullopt}, levtype_{std::nullopt} {}

    Mars2marsMatcherException(long param, std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc), param_{std::to_string(param)}, levtype_{std::nullopt} {}
    ...
    void printFrame(const std::string& pad) const override {
        ...
        Mars2marsGenericException::printFrame(pad);
        if (param_) {
...
class Mars2marsValidationException : public Mars2marsGenericException {
public:

    Mars2marsValidationException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc) {}
};
...
class Mars2marsTableException : public Mars2marsGenericException {
public:

    Mars2marsTableException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc) {}
};
...
class Mars2marsDeductionException : public Mars2marsGenericException {
public:

    Mars2marsDeductionException(std::string reason, const eckit::CodeLocation& loc = eckit::CodeLocation()) :
        Mars2marsGenericException(reason, loc) {}
};
...
///
/// - `MARS2MARS_CONCEPT_THROW`
/// - `MARS2MARS_CONCEPT_RETHROW`
///
/// Those macros automatically inject compile-time metadata.
class Mars2marsConceptException : public Mars2marsGenericException {
public:

    Mars2marsConceptException(std::string name, std::string variant, std::string stage, std::string section,
...
/// via `printFrame()`.
class Mars2marsEncoderException : public Mars2marsGenericException {
public:

    Mars2marsEncoderException(std::string reason, std::string marsDict_json, std::string parDict_json,
...
/// - Section
///
/// and rethrows a `Mars2marsConceptException` while preserving
/// the nested exception chain.
#define MARS2MARS_CONCEPT_RETHROW(CONCEPTNAME, MESSAGE)                                             \
    std::throw_with_nested(Mars2marsConceptException(std::string(CONCEPTNAME##Name),                \
...
///
/// Same as `MARS2MARS_CONCEPT_RETHROW`, but without nesting.
/// Intended for initial throw sites.
#define MARS2MARS_CONCEPT_THROW(CONCEPTNAME, MESSAGE)                                                                  \
    do {                                                                                                               \
        throw Mars2marsConceptException(std::string(CONCEPTNAME##Name), std::string(CONCEPTNAME##TypeName<Variant>()), \
                                        std::to_string(Stage), std::to_string(Section), MESSAGE, Here());              \
    } while (0)
