...
inline eckit::PathName errorStackDirectory(const metkit::mars2mars::Options& opts) {
    return eckit::PathName(opts.errorStackPath).fullName();
}

/// @brief Build a unique error-stack file path for an API entry point.
inline eckit::PathName makeErrorStackFilePath(const metkit::mars2mars::Options& opts, const std::string& apiName) {
...
/// @brief Serialize a mars2mars exception stack to a stream.
inline void writeStructuredExceptionFrame(const std::exception& exception, std::ostream& out, std::size_t level,
    ...
    if (const auto* me = dynamic_cast<const Mars2marsGenericException*>(&exception)) {
...
inline void writeMars2MarsExceptionStackToFile(const Mars2marsGenericException& exception, const std::string& apiName,
    ...
    if (!out) {
        throw eckit::UserError("Mars2Mars conversion failed, but the error stack file could not be opened: `" +
...
/// @brief Print a mars2mars exception stack to standard error.
inline void writeMars2MarsExceptionStackToStdErr(const Mars2marsGenericException& exception, const std::string& apiName,
                                                 const eckit::CodeLocation& loc = eckit::CodeLocation()) {
...
inline std::string buildPublicErrorMessage(const InnermostExceptionInfo& inner,
    ...
    os << "Mars2Mars conversion failed. "
...
Result withMars2MarsApiErrorHandling(const std::string& apiName, const metkit::mars2mars::Options& opts,
    ...
    try {
        ...
        const bool printErrorStackToStdErr =
            opts.printErrorStackToStdErr || detail::envVarEnablesStackMode("MARS2MARS_PRINT_STACK_TO_STDERR");
        const bool saveErrorStack =
            opts.saveErrorStack || detail::envVarEnablesStackMode("MARS2MARS_WRITE_STACK_TO_FILE");

        if (printErrorStackToStdErr) {
            detail::writeMars2MarsExceptionStackToStdErr(e, apiName, loc);
        }
