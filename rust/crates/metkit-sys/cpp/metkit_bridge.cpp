...
bool MarsRequestWrapper::empty() const { return request_.empty(); }

size_t MarsRequestWrapper::count() const { return request_.count(); }
...
void MarsRequestWrapper::set_values(rust::Str key,
...
std::unique_ptr<MarsRequestWrapper>
MarsRequestWrapper::extract(rust::Str category) const {
  return std::make_unique<MarsRequestWrapper>(
      request_.extract(std::string(category)));
}

std::unique_ptr<MarsRequestWrapper>
MarsRequestWrapper::expand(bool inherit, bool strict) const {
...
void CodesHandleWrapper::set_double_array(rust::Str key,
                                          rust::Slice<const double> values) {
  handle_->set(std::string(key),
               metkit::codes::Span<const double>(values.data(), values.size()));
}
...
std::unique_ptr<CodesHandleWrapper>
codes_handle_from_message(rust::Slice<const uint8_t> data) {
  return std::make_unique<CodesHandleWrapper>(
      metkit::codes::codesHandleFromMessageCopy(
          metkit::codes::Span<const uint8_t>(data.data(), data.size())));
}

std::unique_ptr<CodesHandleWrapper> codes_handle_from_file(rust::Str path) {
  return std::make_unique<CodesHandleWrapper>(
      metkit::codes::codesHandleFromFile(std::string(path),
                                         metkit::codes::Product::GRIB));
}

std::unique_ptr<CodesHandleWrapper>
codes_handle_from_file_at_offset(rust::Str path, int64_t offset) {
  return std::make_unique<CodesHandleWrapper>(
      metkit::codes::codesHandleFromFile(std::string(path),
                                         metkit::codes::Product::GRIB, offset));
}

std::unique_ptr<CodesHandleWrapper> codes_handle_from_sample(rust::Str sample) {
  return std::make_unique<CodesHandleWrapper>(
      metkit::codes::codesHandleFromSample(std::string(sample)));
}
...
size_t HyperCubeWrapper::size() const { return cube_->size(); }

size_t HyperCubeWrapper::count() const { return cube_->count(); }

size_t HyperCubeWrapper::count_vacant() const { return cube_->countVacant(); }
...
size_t
HyperCubeWrapper::field_ordinal(const MarsRequestWrapper &request) const {
  return cube_->fieldOrdinal(request.inner());
}

std::unique_ptr<HyperCubeWrapper>
hypercube_create(const MarsRequestWrapper &request) {
...
std::unique_ptr<MarsRequestWrapper> request_create(rust::Str verb) {
  return std::make_unique<MarsRequestWrapper>(
      metkit::mars::MarsRequest(std::string(verb)));
}

std::unique_ptr<MarsRequestWrapper>
request_from_message(const eckit_bridge::MessageWrapper &msg) {
  return std::make_unique<MarsRequestWrapper>(
      metkit::mars::MarsRequest(msg.inner()));
}

std::unique_ptr<MarsRequestWrapper>
request_decode(eckit_bridge::StreamWrapper &stream) {
  return std::make_unique<MarsRequestWrapper>(
      metkit::mars::MarsRequest(stream.inner()));
}

std::unique_ptr<eckit_bridge::DataHandleWrapper>
mars_request_handle(const MarsRequestWrapper &request,
...
size_t ParsedRequestsWrapper::count() const { return requests_.size(); }

std::unique_ptr<MarsRequestWrapper>
ParsedRequestsWrapper::at(size_t index) const {
  return std::make_unique<MarsRequestWrapper>(requests_.at(index));
}

std::unique_ptr<ParsedRequestsWrapper> parse_requests(rust::Str input,
                                                      bool strict) {
...
void request_environment_init(rust::Vec<rust::String> keys,
                              rust::Vec<rust::String> values) {
...
size_t wind_family_count() { return metkit::ParamID::getWindFamilies().size(); }

rust::String wind_family_u(size_t index) {
  return rust::String(
      std::string(metkit::ParamID::getWindFamilies().at(index).u_));
}

rust::String wind_family_v(size_t index) {
  return rust::String(
      std::string(metkit::ParamID::getWindFamilies().at(index).v_));
}

rust::String wind_family_vo(size_t index) {
  return rust::String(
      std::string(metkit::ParamID::getWindFamilies().at(index).vo_));
}

rust::String wind_family_d(size_t index) {
  return rust::String(
      std::string(metkit::ParamID::getWindFamilies().at(index).d_));
}
