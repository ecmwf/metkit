...
BUDGET_MERGE = 30.0
BUDGET_SPLIT = 5.0
BUDGET_SPLIT_WIDE = 30.0
SPLIT_WIDE_COUNT = 100   # fewer requests: each yields 3×4×3×5 = 180 parts
...
def test_split():
    # split() does not require the language engine: pure structural operation.
    # _selection() carries 3 step values, so each split yields 3 requests.
    requests = _requests()
    expected_output = REQUEST_COUNT * 3

    with timed("split()", BUDGET_SPLIT, expected_output):
        for request in requests:
            request.split(["step"])


def test_split_wide():
    # 4-key Cartesian product: step(3) × param(4) × levtype(3) × type(5) = 180 parts/request.
    _STEPS = list(range(0, 13, 6))       # 3 values
    _PARAMS = [129, 130, 131, 132]       # 4 values
    _LEVTYPES = ["sfc", "pl", "ml"]      # 3 values
    _TYPES = ["an", "fc", "cf", "pf", "em"]  # 5 values
    _SPLIT_KEYS = ["step", "param", "levtype", "type"]
    _PARTS_PER_REQUEST = len(_STEPS) * len(_PARAMS) * len(_LEVTYPES) * len(_TYPES)
    ...
    requests = [MarsRequest("retrieve", selection) for _ in range(SPLIT_WIDE_COUNT)]
    expected_output = SPLIT_WIDE_COUNT * _PARTS_PER_REQUEST

    with timed("split() 4-key wide", BUDGET_SPLIT_WIDE, expected_output):
        for request in requests:
            parts = request.split(_SPLIT_KEYS)
        assert len(parts) == _PARTS_PER_REQUEST
